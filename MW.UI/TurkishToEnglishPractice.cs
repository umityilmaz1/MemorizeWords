...
namespace MW.UI
{
    public partial class TurkishToEnglishPractice : Form
    {
        WordService _wordService;
        List<Word> _wordList;
        int count = 0;
        public TurkishToEnglishPractice(List<Word> wordList)
        {
            InitializeComponent();
        ...
        private void TurkishToEnglishPractice_Load(object sender, EventArgs e)
        {
            List<Word> wordList = _wordList;
            for (int i = 0; i < wordList.Count; i++)
            {
                dictPack = new string[2];
                dictPack[0] = wordList[i].TurkishText;
                dictPack[1] = wordList[i].EnglishText;
                words.Add(dictPack);
            }
